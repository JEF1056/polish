...
function generate(event) {
    ...
    _input_ids = tf.tensor2d([input_ids], null, 'int32').pad([[0, 0], [0, max_len-input_ids.length]])
    _attention_mask = tf.ones([1, input_ids.length], 'int32').pad([[0, 0], [0, max_len-input_ids.length]])
    ...
    while (decoder_input_ids.length < 100) {
        ...
        _decoder_input_ids = tf.tensor2d([decoder_input_ids], null, 'int32').pad([[0, 0], [0, max_len-decoder_input_ids.length]])
        _decoder_attention_mask = tf.ones([1, decoder_input_ids.length], 'int32').pad([[0, 0], [0, max_len-decoder_input_ids.length]])

        // run model logit generation
        const _logits = tf.tidy(() => model.execute({
        ...
        const preds = tf.slice(logits, [0, decoder_input_ids.length-1,0], [1, 1, logits.shape[2]]).squeeze().div(temperature); // Take logits, slice to only the layer for the next value and apply temperature; lower temperature = bigger disparity between values, larger temperature = smaller disparity between values
        ...
        const selected = tf.tidy(() => tf.multinomial(softmax, 1, null, true)).dataSync(); // use a multinomial distribution to select a random index based on the softmax probabilities
